...
public class UpdaterPlugin extends Plugin {
    ...
    public void downloadUpdate(PluginCall call) {
        ...
        File dest = new File(ctx.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), "quarc-translate-update.apk");
        if (dest.exists()) dest.delete();

        DownloadManager.Request req = new DownloadManager.Request(Uri.parse(url));
        req.setTitle("Quarc Translate " + version);
        req.setDescription("Downloading update…");
        req.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
        req.setDestinationInExternalFilesDir(ctx, Environment.DIRECTORY_DOWNLOADS, "quarc-translate-update.apk");
    ...
    private void installApk(Context context) {
        File file = new File(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), "quarc-translate-update.apk");
